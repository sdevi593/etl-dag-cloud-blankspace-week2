...
PY_FILE = (
    f'gs://{COMPOSER_BUCKET_NAME}/dataflow-functions/CSV_to_BigQuery.py' if Variable.get('ENVIRONMENT') == 'production'
    else f"{os.path.dirname(configuration.conf.get('core', 'dags_folder'))}/dataflow-functions/CSV_to_BigQuery.py")
PROJECT_ID = Variable.get('PROJECT_ID')
GCS_TEMP_LOCATION = Variable.get('GCS_TEMP_LOCATION')
GCS_STG_LOCATION = Variable.get('GCS_STG_LOCATION')
INPUT_PATH = f'gs://{BUCKET_NAME}/keyword_search'
